	...
	*/				
	class xlsws_admin_maintenance extends xlsws_admin {
		...
		protected function UpgradeWS(){
			...
			//Prior to 2.1.4, we didn't have a database key so we still have to go from the beginning
			if (_xls_get_conf('DATABASE_SCHEMA_VERSION', '213')<214)
			{
	  ...
	  KEY `idxExpires` (`uxtExpires`)
			...
			if (_xls_get_conf('DATABASE_SCHEMA_VERSION', '0')==214)
			{
		...
		protected function backupDB(){
			...
			$this->arrMPnls['backupDB']->Text = date('Y-m-d H:i:s ') . _sp("Backup databases") . "<br/>";
			...
			while ($tables = $tables_query->FetchRow()) {
				...
				$this->arrMPnls['backupDB']->Text .= date('Y-m-d H:i:s ') . _sp("Backing up table $table\n"). "<br/>";
